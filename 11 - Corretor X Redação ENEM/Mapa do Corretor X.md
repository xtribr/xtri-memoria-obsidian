# Mapa do Corretor X

Status: ativo

Este vault é o cérebro isolado do projeto Corretor X, focado em correção pedagógica de redações do ENEM.

## Núcleo

- [[Instruções do Corretor X]]
- [[Manual Oficial ENEM 2025]]
- [[Matriz de Competências ENEM]]
- [[Padrões Nota 1000]]
- [[Protocolo de Correção]]
- [[Prompt Operacional]]
- [[Fontes do Corretor X]]
- [[Backlog do Corretor X]]
- [[00 - Mapa do Cérebro 1000]]

## HTML principal

- [Abrir painel HTML](index.html)
- [Manual oficial em HTML](manual-oficial.html)
- [Competências em HTML](competencias.html)
- [Padrões nota 1000 em HTML](padroes-nota-1000.html)
- [Protocolo em HTML](protocolo-correcao.html)
- [Prompt em HTML](prompt-corretor.html)
- [Fontes em HTML](fontes.html)

## Regra central

A cartilha oficial do Inep é a referência normativa. As redações nota 1000 são amostras de padrão de alto desempenho, não regras oficiais nem modelos para cópia.

## Próxima fase

O [[00 - Mapa do Cérebro 1000]] registra casos reais, erros do corretor e decisões de aprendizado para calibrar a correção ao longo do tempo.
